...
export default function CheckoutPage() {
  ...
  return (
    <div className="min-h-screen bg-floreer-bg grid md:grid-cols-2">
      ...
      {/* Formulário — só e-mail, direto para o pagamento */}
      <div className="p-8 md:p-14 flex items-center">
        <div className="w-full max-w-sm">
          <h2 className="font-serif text-2xl text-floreer-dark mb-2">Finalizar compra</h2>
          ...
          <form onSubmit={handleCompra} className="flex flex-col gap-4">
            <div>
              <label className="block text-[11px] text-floreer-muted tracking-wide mb-1.5">SEU E-MAIL</label>
              <input
                type="email"
                required
                value={email}
              ...
              />
              <p className="text-[11px] text-floreer-muted mt-1.5">É neste e-mail que enviaremos seu acesso. Confira se está correto.</p>
            </div>

            <button
              type="submit"
              disabled={loading}
              className="btn-gold w-full text-center disabled:opacity-60 mt-1"
            >
              {loading ? "Redirecionando..." : `Ir para o pagamento — ${cursoData.precoFormatado}`}
            </button>
          </form>
          ...
          <p className="text-center text-xs text-floreer-muted mt-5">
            Já tem conta?{" "}
